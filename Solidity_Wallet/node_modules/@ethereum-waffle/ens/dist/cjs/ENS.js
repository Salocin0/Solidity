...
async function createResolver(signer, ens) {
    const resolver = await (0, utils_1.deployContract)(signer, resolver_1.PublicResolver, [ens.address]);
...
async function createReverseRegistrar(signer, ens, resolver) {
    const reverseRegistrar = await (0, utils_1.deployContract)(signer, ens_1.ReverseRegistrar, [ens.address, resolver.address]);
...
async function deployENS(signer) {
    const ens = await (0, utils_1.deployContract)(signer, ens_1.ENSRegistry, []);
...
class ENS {
    ...
    async createTopLevelDomain(domain) {
        ...
        this.registrars = {
            ...this.registrars,
            [domain]: await (0, utils_1.deployContract)(this.signer, ens_1.FIFSRegistrar, [this.ens.address, node])
        };
    ...
    async createSubDomainNonRecursive(domain) {
        const { label, node, decodedRootNode } = (0, utils_1.getDomainInfo)(domain);
        ...
        const registrar = await (0, utils_1.deployContract)(this.signer, ens_1.FIFSRegistrar, [this.ens.address, node]);
    ...
    async createDomain(domain, options) {
        try {
            (0, utils_1.getDomainInfo)(domain);
    ...
    async createSubDomain(domain, options) {
        const { decodedRootNode } = (0, utils_1.getDomainInfo)(domain);
    ...
    async setAddressNonRecursive(domain, address) {
        const { node, label, decodedRootNode } = (0, utils_1.getDomainInfo)(domain);
